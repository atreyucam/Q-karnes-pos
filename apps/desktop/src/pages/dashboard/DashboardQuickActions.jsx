      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {quickActions.map(({ id, title, description, to, Icon }) => (
          <button
            ...
            type="button"
            onClick={() => navigate(to)}
            className="group rounded-[22px] border border-[var(--color-border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.96)_0%,rgba(246,244,239,0.82)_100%)] p-4 text-left shadow-[var(--shadow-sm)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[color:var(--color-border-strong)] hover:shadow-[var(--shadow-md)]"
          >
            <div className="flex items-start justify-between gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[color-mix(in_oklab,var(--color-brand-soft)_80%,white_20%)] text-[var(--color-brand)] shadow-sm">
