...
export default function StatusChip({ status, tone, className, children }) {
  const resolvedClass = tone ? `ui-chip-${tone}` : getStatusClasses(status);
  const label = children || formatStatusLabel(status);
  return <span className={clsx('ui-chip', resolvedClass, className)}>{label}</span>;
}
...
export function TipoBadge({ tipo, className, children }) {
  const resolvedClass = getTipoClasses(tipo);
  return <span className={clsx('ui-chip', resolvedClass, className)}>{children || formatStatusLabel(tipo)}</span>;
}
